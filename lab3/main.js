...
function calculateWeights(normalizedMatrix) {
  ...
  var weightsSumm = weights.reduce(function(prev, curr) {
    return prev + curr;
  }, 0);
  
  console.log(weightsSumm);
  
  weights = weights.map(function(weight, index) {
    return { value: weight / weightsSumm, name: 'Z'+(index + 1)};
  });
...
function drawNormalizedMatrix(normalizedMatrix) {
  ...
  rows.forEach(function(row, rowIndex) {
    ...
    cols.forEach(function(col, colIndex) {
      col.innerText = normalizedMatrix[rowIndex][colIndex];
    });
